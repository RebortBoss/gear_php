...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable, JsonSerializable {
	...
	*/
	...
	function fetchAsArray(){
	    $rel=$this->fetch('');
	    return $rel?$rel->getRowArray():null;
    }
	...
	*/
	function fetchPairs($key='', $value = '') {
		...
		if ($value != "") {
			...
			// array_unshift($clone->select, $key);  //Yuri2 TODO  需要加一个列？
		} else {
	...
	public function fetchPairsAsArray(){
        $singles=$this->fetchPairs();
        $rel=[];
	    if ($singles){
            foreach ($singles as $key=> $single){
                $rel[$key]=$single->getRowArray();
            }
        }else{
	        $rel=[];
        }
