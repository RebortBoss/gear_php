...
class QyWeichat
{
    ...
    const MPNEWS_ADD_URL        = '/material/add_mpnews?';//自加 上传图文消息素材。
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	/**
	 * 根据媒体文件ID获取媒体文件(不推荐,适用于直接echo内容客户端下载)
	...
	public function downloadMedia($media_id, $path){
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POSTFIELDS => json_encode(['media_id'=>$media_id]),
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HEADERFUNCTION => function($ch, $head) use($path, &$handle, &$file, $media_id) {
                $tmp = array_map('trim', explode(':', $head, 2));
                if (strtolower($tmp[0]) == 'content-disposition') {
                    if (is_dir($path) && is_writeable($path) && preg_match('/filename=(.*)/', $tmp[1], $matches)) {
                        $file = $media_id . '.' . pathinfo(basename(trim($matches[1], '"')), PATHINFO_EXTENSION);
                        if ($handle = fopen(rtrim($path, '/') . '/' . $file, 'w+')) {
	 ...
	 *
	 * @param $code    string    Oauth2.0或者二次验证时返回的code值
	 * @param $agentid   int|string  跳转链接时所在的企业应用ID，未填则默认为当前配置的应用id
