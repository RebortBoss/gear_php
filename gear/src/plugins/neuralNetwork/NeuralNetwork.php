...
class NeuralNetwork {
	...
	private $errorHandle;
	private $handleBeforeTrain;
	private $handleAfterTrain;
	...
	public function __construct($nodeCount) {
		...
		$this->errorHandle=function ($epoch,$squaredError,$controlset_msg,$slope){
            // buy some time
            set_time_limit(300);
	 ...
	 */
	public function setErrorHandle($errorHandle=null){
		$this->errorHandle=$errorHandle;
	}

    /**
     * @param \Closure $handleBeforeTrain
     */
    public function setHandleBeforeTrain($handleBeforeTrain=null)
    {
        $this->handleBeforeTrain = $handleBeforeTrain;
    }

    /**
     * @param \Closure $handleAfterTrain
     */
    public function setHandleAfterTrain($handleAfterTrain=null)
    {
        $this->handleAfterTrain = $handleAfterTrain;
    }
	...
	public function train($maxEpochs = 500, $maxError = 0.01) {
		...
		if ($this->isVerbose()&& is_callable($this->handleBeforeTrain)) {
			$handle=$this->handleBeforeTrain;
			$handle();
		}
...
//				echo "<td>output = (" . implode(", ", $output) .")</td></tr>";
			...
			if ($this->isVerbose() && is_callable($this->errorHandle)) {
                $handle=$this->errorHandle;
                $handle($epoch,$squaredError,$controlset_msg,$slope);
			}
		...
		if ($this->isVerbose()&& is_callable($this->handleAfterTrain)) {
            $handle=$this->handleAfterTrain;
            $handle();
		}
