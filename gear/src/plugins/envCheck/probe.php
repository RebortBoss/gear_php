...
//MySQL检测
if (isset($_POST['act']) and $_POST['act'] == 'MySQL检测') {
...
} elseif (isset($_POST['act']) and $_POST['act'] == '函数检测') {
...
<div id="page">
    <div id="header">
        <h1>GearPHP 环境检测</h1>
        <div id="lnmplink2"><a title="项目地址" href="https://git.coding.net/yuri2/gear_php.git"
                               target="_blank">GearPHP</a></div>
    </div>

    <!--服务器相关参数-->
    <table width="100%" cellpadding="3" cellspacing="0">
        <tr>
            <th colspan="4">服务器参数(Server Parameters)</th>
        </tr>
        <tr>
            <td>服务器域名/IP地址</td>
        ...
        <tr>
            <td>服务器标识</td>
            <td colspan="3"><?php if (isset($sysInfo['win_n']) and $sysInfo['win_n'] != '') {
                    echo $sysInfo['win_n'];
                } else {
                    echo @php_uname();
                }; ?></td>
        ...
        <tr>
            <td>管理员邮箱</td>
            <td><?php echo isset($_SERVER['SERVER_ADMIN'])?$_SERVER['SERVER_ADMIN']:'unknown'; ?></td>
            <td>探针路径</td>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th colspan="3">运行环境(Runtime Environment)</th>
        </tr>
        <tr>
        ...
        <tr>
            <td>文件读写权限(file read and write privileges)</td>
            <?php
            $name = md5(microtime());
            $rel=[];
            Yuri2::ergodicDirRecursion(PATH_GEAR,function (){},function ($full_path) use ($name,&$rel){
                $file_path = $full_path . DS . $name . '.tmp';
            ...
            Yuri2::ergodicDirRecursion(PATH_PUBLIC,function (){},function ($full_path) use ($name,&$rel){
                $file_path = $full_path . DS . $name . '.tmp';
                file_put_contents($file_path, $name);
                $read_rel = file_get_contents($file_path);
                unlink($file_path);
            ...
            echo !$rel ? '<td>读写正常</td><td><font color="green">√</font></td>' : '<td>读写失败(failed)['.count($rel).']</td><td><font class="warning" color="red">×</font></td>';
            ?>
        </tr>
        <tr>
            <td>curl支持(curl support)</td>
            <td><?php echo isfun("curl_init", true); ?></td>
            <td><?php echo isfun("curl_init"); ?></td>
        </tr>
        <tr>
            <td width="32%">XML解析支持(XML analytical support)</td>
            <td width="18%"><?php echo isfun("xml_set_object", true); ?></td>
            <td width="18%"><?php echo isfun("xml_set_object"); ?></td>
        </tr>
        <tr>
            <td>session支持(session support)</td>
            <td><?php echo isfun("session_start", true); ?></td>
            <td><?php echo isfun("session_start"); ?></td>
        </tr>
        <tr>
            <td>GD库支持(GD support)</td>
            <td>
                <?php
        ...
        <tr>
            <td>imagettftext文字水印(imagettftext support)</td>
            <td>
                <?php
        ...
        <tr>
            <td>正则表达式函数库(Regular expression library)</td>
            <td><?php echo isfun("preg_match", true); ?></td>
            <td><?php echo isfun("preg_match"); ?></td>
        </tr>
        <tr>
            <td>iconv编码转换(iconv code conversion)</td>
            <td><?php echo isfun("iconv", true); ?></td>
            <td><?php echo isfun("iconv"); ?></td>
        </tr>
        <tr>
            <td>mbstring支持(mbstring support)</td>
            <td><?php echo isfun("mb_eregi", true); ?></td>
            <td><?php echo isfun("mb_eregi"); ?></td>
        </tr>
        <tr>
            <td>sockets扩展(sockets support)</td>
            <td><?php echo isfun("socket_create_listen", true); ?></td>
            <td><?php echo isfun("socket_create_listen"); ?></td>
        </tr>
        <tr>
            <td>mysql基本扩展(mysql basic support)</td>
            <td><?php echo isfun("mysql_close", true); ?></td>
            <td><?php echo isfun("mysql_close"); ?></td>
        </tr>
        <tr>
            <td>PDO扩展(PDO support)</td>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] . "#bottom"; ?>" method="post">
        ...
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            <tr>
                <th colspan="2">手动检测(Manual Detection)</th>
            </tr>
            <tr>
                <td width="60%">
            ...
            <?php
            if (isset($_POST['act']) and $_POST['act'] == '函数检测') {
            ...
            ?>
            <tr>
                <th colspan="2">环境确认(Environment Confirm)</th>
            </tr>
            <tr>
                <td>确认以正常使用GearPHP。如果环境检测有不通过的项，对于您的使用可能会有影响，建议修复所有不通过的检测项后再确认使用。如果确认后想重新见到该页面，请将 plugins/envCheck/lock
                    文件删除。
                </td>
                <td width="25%">
                    <input onclick="
                ...
                if (len>0){
                    if (!confirm('发现'+len+'项环境检测未通过，确定要忽略吗？')){
                        return false;
                           ...
                           class="btn" type="submit" name="act" align="right" value="环境确认(Environment Confirm)"/>
                </td>
            </tr>
        </table>
        <?php
        if (isset($_POST['act']) and $_POST['act'] == '环境确认(Environment Confirm)') {
    ...
    <div id="footer">
        <p>
            <span><a title="项目地址" href="https://git.coding.net/yuri2/gear_php.git" target="_blank">GearPHP</a>是一款以自由组合插件为特色的php框架</span>
            <br/>
            <span>更多内容请参阅<a href="http://www.kancloud.cn/yuri2/gear" target="_blank">GearPHP开发文档</a>。</span>
        </p>
