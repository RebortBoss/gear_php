...
class Yuri2
{
    ...
    const VAL_NOT_SET = 'a4b921bd085f562c60f192750f090693';
    const ENC_KEY = 'naples'; //静态类缓存，将大大提高函数执行效率。例如多次判断当前IP，其实只要判断一次，然后把IP缓存至此。
    /** 内部缓存 begin---------------------------------------------------------------------------------------------------------------------- */
     ...
     * @author yuri2
     */
    public static function arrGetSet(&$hd, $path = '', $value1 = self::VAL_NOT_SET, $value2 = null)
    {
        $arrPath = is_array($path) ? $path : self::explodeWithoutNull('.', $path);
        if ($value1 === self::VAL_NOT_SET) {
     ...
     */
    public static function explodeWithoutNull($delimiter, $str)
    {
     ...
     * @return string
     */
    static function smarterEcho($var, $filters = '|', $is_echo = true)
    {
     ...
     * @return string
     */
    static function Camel_to_Hyphen($CamelCase)
    {
        return strtolower(preg_replace(
            array('/(?<=[a-z])([A-Z])/', '/\-\-/'),
            array('-$1', '-'),
     ...
     * @return string
     */
    static function Hyphen_to_Camel($str, $ucfirst = true)
    {
    ...
    {
        if (self::isCached('getHttpType')) {
            return self::useCache('getHttpType');
        }
     ...
     * @author yuri2
     */
    private static function useCache($key, $value = self::VAL_NOT_SET)
    {
        if ($value !== self::VAL_NOT_SET) {
    ...
    {
        if (self::isCached('getHost')) {
            return self::useCache('getHost');
        }
    ...
    {
        if (self::isCached('isAjax')) {
            return self::useCache('isAjax');
        }//直接返回缓存值
    ...
    {
        if (self::isCached('isPost')) {
            return self::useCache('isPost');
        }//直接返回缓存值
    ...
    {
        if (self::isCached('isGet')) {
            return self::useCache('isGet');
        }//直接返回缓存值
    ...
    {
        if (self::isCached('isPjax')) {
            return self::useCache('isPjax');
        }//直接返回缓存值
     ...
     * @return array
     */
    public static function arrayIndexToAssoc($rows, $name_id)
    {
        $rel = [];
        foreach ($rows as $row) {
     ...
     * @param bool $flag 设为true，则不会抛出异常，返回 self::VAL_NOT_SET
     * @return mixed
     * @author yuri2
     */
    public static function invokeMethod($method, $vars = [], $obj = null, $flag = false)
    {
        ...
        if ($args === self::VAL_NOT_SET and $flag) {
            return self::VAL_NOT_SET;
        }
     ...
     * @param $sizeResult int 请勿指定
     * @return int $sizeResult 大小（字节）
     */
    public static function getDirSize($dir,$sizeResult=0)
    {
        $dir = self::autoSysCoding($dir);
        $handle = opendir($dir);
    ...
    public static function ergodicDir($dir, $callable)
    {
        ...
        while ($file = readdir($dh)) {
            if ($file != "." && $file != "..") {
                ...
                $file = self::autoEncoding($file);
                $rel[] = $callable($file, $full_file);
            }
        }
        closedir($dh);
        return $rel;
     ...
     * 递归遍历目录及之下的文件
     * @param $dir string 目标目录
     * @param callable|string $funcFile
        ...
        while ($file = readdir($dh)) {
            if ($file != "." && $file != "..") {
                ...
                if (is_dir($fullPath)) {
                    ...
                    $relFile = $funcFile ? $funcFile($fullPath) : true;
                }
            }
        }
        closedir($dh);
     ...
     * @author yuri2
     */
    public static function arrPublic($target, $value = self::VAL_NOT_SET)
    {
        ...
        while (false !== ($file = readdir($dir))) {
            if (($file != $s) && ($file != '..')) {
                if (is_dir($src . $s . $file)) {
                    if (!self::recurseCopy($src . $s . $file, $dst . $s . $file, 'not_first')) {
                        return false;
                    }
                } else {
     ...
     */
    public static function http_get($url,$timeout=9)
    {
        $oCurl = curl_init();
        if (stripos($url, "https://") !== FALSE) {
            ...
            curl_setopt($oCurl, CURLOPT_TIMEOUT,$timeout ); //CURL_SSLVERSION_TLSv1
        }
     ...
     * @param int $num
     */
    public static function httpStateCode($num)
    {
        $http = array(
     ...
     */
    public static function fullArray($arr, &$rel, $str = '')
     ...
     */
    public static function echoTagP($content)
    {
        echo "<p>$content</p>";
    }
    ...
    {
        if (self::isCached('getIp')) {
            return self::useCache('getIp');
        }//直接返回缓存值
        $remote_addr=self::setOrDefault($_SERVER["REMOTE_ADDR"],'unknown');
        $user_IP = (isset($_SERVER["HTTP_VIA"]) and $_SERVER["HTTP_VIA"]) ? $_SERVER["HTTP_X_FORWARDED_FOR"] : $_SERVER["REMOTE_ADDR"];
        $user_IP = ($user_IP) ? $user_IP : $remote_addr;
     ...
     * @return mixed
     */
    function setOrDefault(&$var,$default=null){
    ...
    {
        if (self::isCached('getUA')) {
            return self::useCache('getUA');
        }
    ...
    public static function createDir($path, $mode = 0775)
    {
        ...
        if (is_dir($path)) {  //判断目录存在否，存在不创建
            ...
            $re = @mkdir($path, $mode, true); //第三个参数为true即可以创建多级目录
            if ($re) {
                return true;
            } else {
                return false;
    ...
    {
        if (self::isCached('isMobile')) {
            return self::useCache('isMobile');
        }
    ...
    /**
     * 设置错误报告级别 1-10级，10级为报告最详细
     * @param $level int
     */
    public static function setErrorReportLevel($level){
        //此处修改错误报告级别
        $report_level=1;
        for($i=0;$i<$level;$i++){
            $report_level|=pow(2,$i);
     ...
     * 线性数据标准化（常用于神经网络样本）
     * @param $D int|float
     * @param $Dmin int|float
     * @param $Dmax int|float
     * @param $Imin int|float
     * @param $Imax int|float
     * @return float
     */
    public static function normalize($D, $Dmin=-1, $Dmax=1, $Imin=-1, $Imax=1){
        return  $Imin + ($Imax-$Imin)*($D-$Dmin)/($Dmax-$Dmin);
    }
     ...
     * @param $expire int
     * */
    public static function headerSendCachePage($expire=300){
        header('Cache-Control: max-age='.$expire);
        header('Pragma: max-age='.$expire);
    }
